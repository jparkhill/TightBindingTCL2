...
public:
    ...
    inline void LorentzMarkovRate(const complex<double>& Eps,const complex<double>& g,const complex<double>& l,const  complex<double>& O,const  complex<double>& Beta, complex<double>& gam) const
    {
        ...
        // Sam and I found a bug in this term where C(t) didn't have the right symmetries. It's been fixed. now.
...
// Besides the arguments, this depends on a file which contains J(w) in the DL form.
class TCLMatrices
{
public:
    ...
    //    arma::cx_cube GammaAtEpsilon_low;
    arma::cx_cube u;
    arma::cx_mat eps;
    arma::cx_vec fock;
    arma::cx_mat C;
        ...
        ReadJwFromDisk();
        {
            ...
            try{
                ...
                if (foundrates)
                {
                    ...
                    if(ratestmp.n_rows != No or ratestmp.n_cols != No or ratestmp.n_slices != Nao)
                    {
	...
	{
        if (!is_finite(C_) or !is_finite(f_diag) )
		{
            ...
            for (int m = 0 ; m<Nao; ++m)
            {
                ...
                for (int a = 0; a<No; ++a)
                {
                    for (int  n= 0; n<No; ++n)
                    {
                        ...
                        // Note.. sometimes negative lamdas are kicked out in ReadJwFromDisk. For now just try this out...
...
//        cx_vec tmp3=FermiDirac(n_e,fock);
//        real(tmp3).st().print(" Fermi Dirac Populations");
	...
	void ReadJwFromDisk(void)
	{
		...
		{
            ifstream reader;
            cout << " Looking for SD.csv, and Drude parameters in atomic units. " << endl;
            if (low)
...
#pragma omp parallel for private(um,uG,uGst,uG_d,uGst_nd,uhr,hru) shared(i1,i2,tmp) schedule(static)
    ...
    void ContractGammaRhoMThreads_diag(arma::cx_mat& OutRho,const arma::cx_mat& Rho_) const
	{
        ...
        // The fancy pants chaining of operator templates (%,-= etc.) in arma::cx_vec have not been well debugged. Causes all sorts of garbage segfaults.
        // My solution is just to use cx_mat(n,1) instead.
		cx_mat HoleRho(Rho.n_rows,Rho.n_cols);
		HoleRho.eye();
		HoleRho = HoleRho - Rho;
...
#pragma omp parallel for shared(i1,i2,tmp,Rho,HoleRho,Rhod,HoleRhod) schedule(guided)
    ...
    {
		if (p>=No or q>=No or r>=No or s>=No or p<0 or q<0 or r<0 or s<0)
		{
