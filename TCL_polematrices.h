...
public:
	...
	{
		IsOn = (abs(ImpulseAmp(time)) > pow(10.0,-10.0));
		if (IsOn)
...
public:
	...
	{
		if (ApplyImpulse2)
			return (Amp*sin(Freq*time)*(1.0/sqrt(2.0*M_PI*tau))*exp(-1.0*pow(time-t0,2.0)/(2.0*tau*tau))+
					Amp2*sin(Freq2*time)*(1.0/sqrt(2.0*M_PI*tau2))*exp(-1.0*pow(time-t02,2.0)/(2.0*tau2*tau2)));
		else
			return Amp*sin(Freq*time)*(1.0/sqrt(2.0*M_PI*tau))*exp(-1.0*pow(time-t0,2.0)/(2.0*tau*tau));
	}
	...
	void Impulse(cx_mat& arg, const double time,bool& IsOn) const
	{
		...
		if (IsOn)
		{
			if (pol.n_elem <3 or muxo.n_elem+muyo.n_elem+muzo.n_elem != 3*arg.n_elem)
			{
